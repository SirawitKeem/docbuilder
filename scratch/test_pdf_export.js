...
async function run() {
  ...
  const printUrl = `${baseUrl}/print/quotation?data=${encodeURIComponent(encoded)}`;

  console.log("Launching browser for URL:", printUrl);
  ...
  // Get DOM information about rendered pages
  const pageInfo = await page.evaluate(() => {
    ...
    return {
      ...
      cardsDetails: Array.from(cards).map((c, i) => ({
        index: i,
  ...
  console.log("Rendered Page Info in Browser:", JSON.stringify(pageInfo, null, 2));
  ...
  fs.writeFileSync("scratch/test_output.pdf", pdfBuffer);
  console.log("PDF saved to scratch/test_output.pdf! Size:", pdfBuffer.length, "bytes");
