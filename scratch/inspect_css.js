...
async function inspectCss() {
  ...
  const payload = {
    ...
    deliveryTerm: "7 days",
    creditTerm: "30 days",
    billTo: { companyName: "CS LoxInfo", attn: "Sarun" },
  ...
  const info = await page.evaluate(() => {
    const cards = Array.from(document.querySelectorAll(".quotation-document-wrapper > div"));
    return cards.map((c, idx) => {
      ...
      return {
        ...
        pageBreakAfter: style.pageBreakAfter || style.breakAfter,
        textSnippet: c.innerText.substring(0, 100).replace(/\n/g, " "),
      };
    });
  });

  console.log("Computed Card Info:", JSON.stringify(info, null, 2));
  await browser.close();
}
