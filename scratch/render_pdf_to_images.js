...
async function renderPdfPages() {
  ...
  const jsonString = JSON.stringify({
    ...
    billTo: {
      ...
      am: "Narin Rattanavajai / Channel Manager",
    },
    lineItems: [
      {
        id: "item-1",
  ...
  const cards = await page.$$(".quotation-document-wrapper > div");
  console.log("Total Cards in DOM:", cards.length);

  for (let i = 0; i < cards.length; i++) {
    const box = await cards[i].boundingBox();
    console.log(`Card ${i + 1} bounding box:`, box);
    await cards[i].screenshot({ path: `scratch/card_page_${i + 1}.png` });
  }

  await browser.close();
  console.log("Screenshots saved to scratch/card_page_X.png!");
}
