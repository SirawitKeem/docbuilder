...
import NdaPage1 from "./nda/NdaPage1";
import NdaPage2 from "./nda/NdaPage2";
import NdaPage3 from "./nda/NdaPage3";
import NdaPage4 from "./nda/NdaPage4";

const A4_WIDTH = 794;
const A4_HEIGHT = 1123;
const pageComponents = [NdaPage1, NdaPage2, NdaPage3, NdaPage4];

export default function ReviewScreen({ onExport, onSendEmail, exporting }) {
  const { values, setReadOnly } = useDocumentFields();
    ...
    <div className="flex flex-col h-screen bg-gray-100">
      ...
      <div className="flex-1 overflow-auto">
        <div className="max-w-3xl mx-auto px-4 py-8">
          {/* Completion Status */}
          <div
            className={`rounded-card px-5 py-4 mb-6 flex items-center gap-3 ${
              status.isComplete ? "bg-success-100" : "bg-gray-100"
            }`}
          >
            <span
              className={`w-2 h-2 rounded-full ${
                status.isComplete ? "bg-success-600" : "bg-gray-400"
              }`}
            />
            <div>
              <p
                className={`text-sm font-semibold ${
                  status.isComplete ? "text-success-600" : "text-gray-700"
                }`}
              >
                {status.isComplete
                  ? "✓ กรอกข้อมูลครบแล้ว"
                  : `กรอกข้อมูลแล้ว ${status.filled}/${status.total} รายการ`}
              </p>
              <p className="text-xs text-gray-500">
                {status.isComplete
                  ? "พร้อมสำหรับการตรวจสอบเอกสาร"
                  : "กรุณากลับไปกรอกข้อมูลให้ครบก่อนส่งออก"}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-8 pb-12">
            {pageComponents.map((PageContent, i) => (
              <div
                key={i}
                className="bg-white shadow-document mx-auto"
                style={{ width: A4_WIDTH, minHeight: A4_HEIGHT }}
              >
                <div
                  className="px-16 pt-12 pb-8 flex flex-col font-noto-looped"
