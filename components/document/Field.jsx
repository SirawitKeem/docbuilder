...
export default function Field({ id, placeholder, type = "text", minWidth = 4 }) {
  ...
  // โหมด Review — แสดงเป็นข้อความในเนื้อเอกสารจริง ไม่มีกรอบฟอร์ม (Design System ข้อ 18)
  if (readOnly) {
    if (isEmpty) {
  ...
  if (type === "textarea") {
    return (
      <textarea
        ...
        rows={2}
        className={`block w-full my-1 px-2 py-1 rounded border outline-none transition-colors duration-150 resize-none text-inherit font-inherit leading-inherit ${stateClasses}`}
      />
    );
  }
  ...
  return (
    <input
      ...
      onFocus={() => setFocused(true)}
      onBlur={() => setFocused(false)}
      style={{ width: `${Math.max(minWidth, (value || placeholder || "").length + 2)}ch` }}
      className={`inline-block align-baseline px-1.5 py-0.5 mx-0.5 rounded border outline-none transition-colors duration-150 text-inherit font-inherit ${stateClasses}`}
    />
  );
}