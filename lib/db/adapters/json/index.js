...
const defaultData = {
  ...
  settings: { language: "th", currency: "THB", theme: "light" },
};
...
async function readDb() {
  try {
    ...
    const db = {
      ...
      settings: parsed.settings || { language: "th", currency: "THB", theme: "light" },
    };
    if (!parsed.categories || !parsed.customTemplates || !parsed.settings || parsed.quotations) {
      await writeDb(db);
    }
    return db;
  } catch (err) {
...
export const jsonSettingsRepo = {
  async get() {
    const db = await readDb();
    return db.settings || { language: "th", currency: "THB", theme: "light" };
  },
  async update(patch) {
    const db = await readDb();
    const current = db.settings || { language: "th", currency: "THB", theme: "light" };
